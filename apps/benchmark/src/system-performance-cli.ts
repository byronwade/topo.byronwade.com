...
import { runSystemPerformance } from "./system-performance.js";
...
async function main(): Promise<void> {
  ...
  const outputPath = path.resolve(
    valueAfter(args, "--output") ??
      "artifacts/benchmarks/performance-system.json",
  );
  const report = await runSystemPerformance({
    hotIterations: integerAfter(args, "--hot-iterations"),
    coldIterations: integerAfter(args, "--cold-iterations"),
    adapterRoutes: integerAfter(args, "--adapter-routes"),
    daemonRoutes: integerAfter(args, "--daemon-routes"),
    cliEntryPath: valueAfter(args, "--cli-entry"),
  });
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      results: Object.fromEntries(
        report.results.map((result) => [result.id, result.p95Ms]),
      ),
      comparisonStatus,
...
main().catch((error: unknown) => {
  process.stderr.write(
    `Topo system benchmark failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
